...
/* harmony import */ var _modules_functions_js__WEBPACK_IMPORTED_MODULE_0__ = __webpack_require__(1);
      ...
      /*------------------------------
      Инициализация product-слайдеров внутри карточек
      ------------------------------*/
      function initProductImageSliders(container) {
         container.querySelectorAll('.catalog__product-slider').forEach((productSlider) => {
            const slides = productSlider.querySelectorAll('.swiper-slide');
            const pagination = productSlider.closest('.catalog__product')?.querySelector('.catalog__product-pagination');

            if (slides.length > 0 && !productSlider.classList.contains('swiper-initialized')) {
      ...
      function handleAfterVisibility(swiperInstance, sliderElement) {
         if (swiperInstance.isEnd) {
            ...
            sliderElement.classList.remove('hide-after');
         }
      }
      ...
      const initMainSwiper = debounce(() => {
         const catalogSliders = document.querySelectorAll('.catalog__products-slider');

         catalogSliders.forEach((catalogSlider) => {
            const slides = catalogSlider.querySelectorAll('.swiper-slide');

            if (window.innerWidth <= 767) {
               if (!catalogSlider.swiper) {
                  catalogSlider.swiper = new Swiper(catalogSlider, {
                     slidesPerView: 1.3,
                     spaceBetween: 12,
                     speed: 800,
                     on: {
                        init: function () {
                           handleAfterVisibility(this, catalogSlider);
                        },
            ...
            } else if (catalogSlider.swiper) {
               catalogSlider.swiper.destroy();
               catalogSlider.swiper = null;
            }
         });
      }, 200);
      ...
      /*------------------------------
      Табы: переключение категорий по клику
      ------------------------------*/
      document.addEventListener('DOMContentLoaded', () => {
         const menuButtons = document.querySelectorAll('.catalog__menu-slide');
         const productSliders = document.querySelectorAll('.catalog__products-slider');

         const showCategory = (category) => {
            const productsContainer = document.querySelector('.catalog__products');

            productSliders.forEach((slider) => {
               const isCurrent = slider.getAttribute('data-category') === category;

               if (isCurrent) {
                  slider.style.display = 'block';
                  initProductImageSliders(slider);
         ...
         menuButtons.forEach((button, index) => {
            button.addEventListener('click', (e) => {
               ...
               if (catalogMenuSwiper) {
                  ...
                  const totalSlides = catalogMenuSwiper.slides.length;
                  const slidesPerView = catalogMenuSwiper.params.slidesPerView;

                  let slideToIndex = index;

                  if (index >= totalSlides - slidesPerView) {
                     slideToIndex = totalSlides - slidesPerView;
                  }

                  catalogMenuSwiper.slideTo(slideToIndex, 300);

                  const sliderEl = document.querySelector('.catalog__menu-slider');
                  if (index === totalSlides - 1) {
                     sliderEl.classList.add('hide-after');
                  } else {
                     sliderEl.classList.remove('hide-after');
                  }
               }
            });
         });
         ...
         const initialButton = document.querySelector('.catalog__menu-slide.active');
         const initialCategory = initialButton?.getAttribute('data-category') || '1';
      ...
      if (moreProductsSlider) {
         const moreProductsSwiper = new Swiper(moreProductsSlider, {
            ...
            speed: 800,
            navigation: {
               nextEl: '.more-products__slider-next',
               prevEl: '.more-products__slider-prev',
            },
            breakpoints: {
               320: {
                  slidesPerView: 1.3,
            ...
            on: {
               init: function () {
                  handleAfterVisibility(this, moreProductsSlider);
               },
               slideChange: function () {
                  handleAfterVisibility(this, moreProductsSlider);
               }
      ...
      document.querySelectorAll('.constructor__options-row').forEach(row => {
         const tips = row.querySelector('.constructor__options-tips');
         const btn = row.querySelector('.tips__btn');
         const close = row.querySelector('.tips__close');

         if (!tips || !btn) return;

         tips.classList.remove('show'); // На всякий случай скрываем

         // === Блокируем активацию радио при клике на кнопку или подсказку ===
         btn.addEventListener('click', (e) => {
            ...
            if (isMobile) {
               ...
               document.querySelectorAll('.constructor__options-tips.show').forEach(t => {
                  if (t !== tips) t.classList.remove('show');
               });
         ...
         if (isMobile) {
            ...
            btn.addEventListener('mouseleave', () => {
               setTimeout(() => {
                  if (!tips.matches(':hover') && !btn.matches(':hover')) {
                     tips.classList.remove('show');
                  }
               }, 100);
            });
            ...
            tips.addEventListener('mouseleave', () => {
               setTimeout(() => {
                  if (!tips.matches(':hover') && !btn.matches(':hover')) {
                     tips.classList.remove('show');
                  }
